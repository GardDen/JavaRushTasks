...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            @Override
            public String say() {
                StringBuilder woof = new StringBuilder("а");
                for (int j=1; j < i; j++)
                woof.append("а");
                String awake = String.format("%s лает г%sв! %s", getName(), woof, formatter.format(new Date()));
                String sleep = String.format("%s спит.",getName());
                return i < 1     ?     sleep : awake;
            }
